...
```
  ...
  The constant raster values are added to the `swns_stations_df` table with  `extract_constant_rasters_values()`.  A new frame will be made called `swns_stations_df_cr`, which is the original table plus constant raster values.
```{r message=FALSE, warning=FALSE}
swns_stations_df_cr <- extract_constant_raster_values(swns_stations_df, rasters_list)
str(swns_stations_df_cr)
```
