...
<<<<<<< Updated upstream
=======
#install.packages(c('backports','dplyr','raster','lubridate','reshape2','modelr','randamcoloR','rgdal'))
>>>>>>> Stashed changes
library(swnsmodelr)
library(ggplot2)
```
...
<<<<<<< Updated upstream
  The `swnsmodelr` package was designed for users to easily model SWNS weather station data with rasters, stations, input variable(s) and models of their choosing. The package contains functions and weather station data, and input rasters are contained within the package's R project. 
...
=======
                       X   = paste0("d:\\data\\rasters\\20m\\",rasters_names_list, ".tif"))
>>>>>>> Stashed changes
