...
melt
```{r melt dataframe}
swns_stations_melt_df <- reshape2::melt(swns_stations_solar_df, id.vars = c('stationid','date_time','temp_min','temp_max','temp_mean','EASTING','NORTHING'))
...
df <- stations_vars_df %>% 
  ...
  filter(date_time == ymd('2012-06-01'))
df_max <- df  %>%
  group_by(variable) %>%
  slice_max(value) %>%
  ungroup() 
...
#### Define the model
```{r}
df <- swnsmodelr::swns_stations_solar_df %>% add_date_columns() %>% filter(stationid %in% swnsmodelr::nscc_stations_list)
m1 <- gam(temp_mean ~ 
           year + 
           s(yday) + 
           s(east1000m, north1000m) + 
           s(east1000m) + 
           s(north1000m) +
           s(dem1000m) +
           s(cp1000m) +
...
```{r}
resid_df <- swnsmodelr::stations_df %>% 
  filter(stationid %in% swnsmodelr::ext_stations_list) %>%
  add_date_columns()
resid_df <- add_residuals(resid_df,m) %>% select(stationid, resid, date_time)
```
