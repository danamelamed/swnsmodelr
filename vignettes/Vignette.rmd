...
resolutions = c('20m','100m','200m','700m','1000m')
resolutions = c('200m')
swns_stations_df_solar200m <- swns_stations_df %>% 
  filter(between(date_time,ymd('2012-04-01'),ymd('2012-11-30')))
...
df <- extract_temporal_raster_values(solar_irradiance_rasters_df,
                                    swns_stations_df_2012,
                                    paste0('solar',res))
col_name = paste0('solar',res)
swns_stations_df_solar200m <- df %>% mutate( df[length(df)])
...
df <- swns_stations_df %>% add_date_columns() %>%
 ...
 # filter(year == 2012) %>%
  filter(stationid %in% swnsmodelr::nscc_stations_list |
...
Create the raster brick & outputs
```{r, warning = FALSE}
resolution <- 700
out_folder <- 'f:\\output\\daily_mean\\solar_tpi7000m\\'
days <- seq(ymd('2012-07-01'),ymd('2012-07-31'), by = "days")
temp_mean_rasters <- list()
...
#       
#     }else{
#       try(rasters_brick <- raster::addLayer(rasters_brick,raster))
#     }
#     
#     
#   }
#   
# }
# rasters_brick <- raster::addLayer(rasters_brick,tpi7000m700m)
# rasters_brick <- raster::addLayer(rasters_brick,cp700m_10km)
# rasters_brick <- raster::addLayer(rasters_brick,cp700m_2)
# 
# #Solar raster
...
#  tryCatch(
#     expr = {
#     solar_raster <- raster::raster(df$path_field[[1]])
#     names(solar_raster) <- paste0('solar',resolution,'m')
#     rasters_brick <- raster::addLayer(rasters_brick,solar_raster)
#     },
#     error = function(e){print('no solar today') }
# 
# 
#  )
df_today = df %>% filter(date_time = today)
m<- gam(gdd10~s(dem700m,k=30,bs='cr')+s(cp700m,k=5,bs='cr')+s(east700m,north700m,k=20),data=df_today,method='REML',family=gaussian())
gam_raster <- raster::predict(rasters_brick2,m)
plot(gam_raster)


writeRaster(gam_raster, paste0(out_folder,
                               'daily_mean_',
                                today,
                               '.tif'),
            overwrite=TRUE)
