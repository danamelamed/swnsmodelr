...
The temperature data is a dataframe of daily temperature readings at stations in South-West Nova Scotia. The fields are the date (`date_time`), the station location (`EASTINGS` and `NORTHINGS`), and temperature minimum and maximum (`temp_min` and `temp_max`, respectively).
...
Calculate daily temperature means
```{r}
