extract_daily_spvs  <- function(df,
  ...
  df <- dplyr::filter(df, year == 2012)
  
  df_yday_bind <- list()
  for(i in seq_along(days)){
    
    print(days[[i]])
    df_yday <- dplyr::filter(df, yday == days[[i]])
    ...
    df_yday_bind[[i]] <- dplyr::bind_rows(dep_var_df)
  }
  df_out <- dplyr::bind_rows(df_yday_bind)
}
