...
for(i in 1:n_weeks){
  ...
  for(j in 1:length(unique(weekly_df$yday))){
    ...
    if(length(solar_irradiance_now[[1]] != 0)){
      ...
      writeRaster(prediction_raster, paste0("Z:\\Dana\\Weekly\\Daily_Temp_Mean_200_5\\temp_mean",
                                            date_now,".tif"),
                  overwrite = TRUE)
    }else{
...
# Create GDDs
temp_mean_df <- make_temporal_raster_df("Z:\\Dana\\Weekly\\Daily_Temp_Mean_200",
                                        start_date,
                                        end_date,
                                        date_chars = c(10,19),
                                        date_format = "%Y-%m-%d")
